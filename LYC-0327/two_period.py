...
class TwoPeriodModel(EquationSystem):
    ...
    def jacobian(self, x: NDArrayFloat, params: Params) -> NDArrayFloat:
        """Machine-precision Jacobian via complex-step differentiation.

        f'(x) ≈ Im(f(x + i·h)) / h  with h ≈ 1e-30 — no subtractive
        cancellation, so the result matches an analytical Jacobian to
        ...
        for j in range(n):
            ...
            x_c[j] += 1j * h
            f_c = self._equations_complex(x_c, params)
            J[:, j] = f_c.imag / h
        return J

    def _equations_complex(self, x: np.ndarray, params: Params) -> np.ndarray:
        """Same equations as equations(), but accepting complex128 inputs."""
        v = dict(zip(self.variable_names, x))
        ...
        varphi_1 = beta_1 * sigma_v2 * (sigma_u2 + sigma_epsilon2) / denom_varphi_psi
        varphi_2 = -beta_1 * sigma_v2 * sigma_u2 / denom_varphi_psi
        psi_1 = rho * sigma_u2 * sigma_epsilon2 / denom_varphi_psi
        psi_2 = rho * beta_1**2 * sigma_v2 * sigma_u2 / denom_varphi_psi

        phi_noise_mix = ((1 - nu) ** 2) * sigma_u2 + (nu**2) * sigma_epsilon2
        denom_phi = sigma_v2 * phi_noise_mix * (
            beta_2 - rho * beta_1
        ) ** 2 + sigma_eta2 * (beta_1**2 * sigma_v2 + phi_noise_mix)
        phi_1 = sigma_v2 * (beta_2 - rho * beta_1) * phi_noise_mix / denom_phi
        phi_2 = (
            sigma_v2
            * (rho * (beta_1 * rho - beta_2) * phi_noise_mix + beta_1 * sigma_eta2)
            / denom_phi
        ...
        eq_beta_1 = beta_1 - (
            (
                2 * total_alpha_2 * total_alpha_1**2
                + total_alpha_1 * total_alpha_2 * total_delta
            )
            / (4 * total_alpha_2 * total_alpha_1 - total_delta**2)
        )
        eq_beta_2 = beta_2 - 0.5 * (total_alpha_2 + beta_1 * drift_ratio_12)
        eq_delta_T = delta_T - 0.5 * (total_delta - drift_ratio_12 * total_alpha_1)

        eq_alpha_I_1 = alpha_I_1 - (
            (
                2 * F_informed * G_informed
                - (delta_T - D_informed) ** 2
                + eq4_linear_term * varphi_1 * G_informed
                - (delta_T - D_informed) * psi_1 * G_informed
            )
            / eq4_common_denom
        )
        eq_gamma_1 = gamma_1 - (
            (
                eq4_linear_term * (varphi_1 * (J_I - 1) * gamma_1 + varphi_2)
                - (delta_T - D_informed)
                * (psi_1 * (J_I - 1) * gamma_1 + psi_2 - (J_I - 1) * gamma_2)
            )
            / eq4_common_denom
        )
        eq_alpha_I_2 = phi_1 * total_alpha_2 + alpha_I_2 / F_informed - 1
        eq_gamma_2 = gamma_2 - (
            -F_informed
            * (phi_2 * J_I * gamma_1 + phi_3)
            / (1 + phi_1 * J_I * F_informed)
        )
        eq_delta_I = delta_I - (
            -F_informed
            * (phi_1 * (D_informed - delta_T) + phi_2 * total_alpha_1)
            / (1 + phi_1 * F_informed)
        )
        eq_alpha_U_1 = alpha_U_1 - alpha_U_1_num * A_mixed / alpha_U_1_den
        eq_alpha_U_2 = alpha_U_2 - (
            B_mixed * (1 - xi_2 * J_I * alpha_I_2) / uninformed_update_den
        )
        eq_delta_U = delta_U - (
            -B_mixed
            * (xi_2 * (J_I * delta_I - delta_T) + xi_1 * total_alpha_1)
            / uninformed_update_den
        )
